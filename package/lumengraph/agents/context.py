...
from lumengraph import config as sys_config
...
class BaseContext:
    ...
    配置优先级：
    1. 运行时配置（RunnableConfig）：最高优先级，直接从函数传入
    ...
    tools:Annotated[list[str],{"__template_metadata__":{"kind":"tool"}}] = field(
    ...
    mcps:Annotated[list[str] | None,{"__template_metadata__":{"kind":"mcps"}}] = field(
        ...
        if get_origin(field_type) is not None:
            ...
            if hasattr(origin, "__name__"):
                if origin.__name__ == "Annotated":
                    # 对于 Annotated 类型，提取基础类型
                    args = get_args(field_type)
                    if args:
                        return cls._get_type_name(args[0])  # 递归处理基础类型
